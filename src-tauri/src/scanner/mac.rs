// macOS installed-app + running-window scanner — the CoreFoundation/
// CoreGraphics half of the process/app discovery pipeline. See `scanner.rs`
// for the module-level overview; output feeds `matcher::match_apps`.
//
// Enumerates running windows via CGWindowListCopyWindowInfo and installed
// apps by scanning the standard /Applications directories + parsing each
// Info.plist. Ported in-process from the former `src/bin/scanner_mac.rs`
// sidecar binary so it feeds the same matcher::match_apps -> APP_CACHE ->
// /search pipeline as the Windows path. Output structs come straight from
// crate::matcher (no JSON hop).
//
// Window titles require Screen Recording permission (macOS 10.15+). Without
// it kCGWindowName is absent; we fall back to the owner-app name so the
// matcher can still correlate windows with installed apps.
