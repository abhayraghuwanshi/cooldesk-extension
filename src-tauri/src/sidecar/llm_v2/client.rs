...
impl ToolCallParser {
    ...
    fn parse_xml_format(response: &str) -> Option<Vec<ParsedToolCall>> {
        ...
        // Simple regex-like matching for <tool>name</tool><args>json</args>
        let tool_start = "<tool>";
        let tool_end = "</tool>";
        let args_start = "<args>";
        let args_end = "</args>";
        ...
        while let Some(ts) = remaining.find(tool_start) {
            let after_ts = &remaining[ts + tool_start.len()..];
            if let Some(te) = after_ts.find(tool_end) {
                let tool_name = &after_ts[..te];
    ...
    pub fn extract_text(response: &str) -> String {
        ...
        while let Some(start) = text.find("<tool>") {
            if let Some(end) = text.find("</args>") {
                text = format!("{}{}", &text[..start], &text[end + "</args>".len()..]);
            } else {
                break;
            }
        }
        ...
        while let Some(start) = text.find("[TOOL:") {
            if let Some(paren_start) = text[start..].find('(') {
                if let Some(paren_end) = text[start + paren_start..].find(')') {
                    let end = start + paren_start + paren_end + 1;
