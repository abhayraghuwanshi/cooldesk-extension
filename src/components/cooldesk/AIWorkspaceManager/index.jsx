...
export default function AIWorkspaceManager({
  ...
  useEffect(() => {
    ...
    if (selectedWorkspaceId && mode === 'edit') {
      ...
      if (workspace) {
        ...
        // Fetch AI-suggested URLs for this workspace
        if (workspace.urls?.length > 0) {
          setRelatedUrlsLoading(true);
          suggestRelatedUrls(workspace)
            .then(urls => {
  ...
  }, [selectedWorkspaceId, mode, workspaces, suggestRelatedUrls]);
  ...
  const handlePromptSubmit = useCallback(async (promptText) => {
    ...
    setMemoryContext(ctx);
    generateSuggestions(promptText, ctx);
  }, [memory, tabs, generateSuggestions]);
