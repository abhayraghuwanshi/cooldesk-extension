...
import { faBolt, faChevronDown, faClock, faDesktop, faExternalLinkAlt, faFolder, faGlobe, faMagic, faPowerOff, faTasks, faThumbtack, faTimes } from '@fortawesome/free-solid-svg-icons';
...
/**
 * FolderCard — a compact folder chip. Browsing happens in the in-app
 * FileManager (opened by the parent via onClick), not in an inline tree: a
...
Click to browse in CoolDesk`}
    >
      ...
      <div className="folder-chip-info">
        <div className="folder-chip-name">{name}</div>
        <div className="folder-chip-path">{shortPath}</div>
      </div>
      {onOpenExternal && (
        <button
          className="folder-chip-action"
          onClick={(e) => { e.stopPropagation(); onOpenExternal(folder); }}
